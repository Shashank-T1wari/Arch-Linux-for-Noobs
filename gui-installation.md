...
Find the line starting with `GRUB_CMDLINE_LINUX_DEFAULT=`. You need to append `nvidia_drm.modeset=1` inside the quotes.
...
3. **Instead of a pure text TTY**, you should now arrive at a graphical login screen (**SDDM**).
4. Before typing your password, look for a "Session" dropdown menu (usually at the bottom left or top left). Ensure **Hyprland** is selected.
